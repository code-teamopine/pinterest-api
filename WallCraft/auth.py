...
async def authenticate_admin(user_username: str, user_password: str) -> None | dict:
    db_obj = Mysql()
    try:
        user_dict = db_obj.select(query_str="select username from user where username = %s and password = %s", query_params=(user_username, hashlib.sha1(bytes(user_password, 'utf-8')).hexdigest()), is_fetch_one=True)
